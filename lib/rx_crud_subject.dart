...
class RxCrudSubject<Created, Read, Updated, Deleted> {
  ...
  late final StreamController<RxEvent<Created, Read, Updated, Deleted>>
  _controller = StreamController.broadcast(sync: sync);

  Stream<RxEvent<Created, Read, Updated, Deleted>> get stream =>
      _controller.stream;
  ...
  void _addCreated(Created value) {
    _controller.add(RxCreateEvent(value: value));
  }

  void _addRead(Read value) {
    _controller.add(RxReadEvent(value: value));
  }

  void _addDeleted(Deleted value) {
    _controller.add(RxDeleteEvent(value: value));
  }

  void _addUpdated(Updated value) {
    _controller.add(RxUpdateEvent(value: value));
  }

  void _addBatchCreated(Iterable<Created> batch) {
    _controller.add(RxBatchCreated(batch: batch));
  }

  void _addBatchRead(Iterable<Read> batch) {
    _controller.add(RxBatchRead(batch: batch));
  }

  void _addBatchUpdated(Iterable<Updated> batch) {
    _controller.add(RxBatchUpdated(batch: batch));
  }

  void _addBatchDeleted(Iterable<Deleted> batch) {
    _controller.add(RxBatchDeleted(batch: batch));
  }
}
